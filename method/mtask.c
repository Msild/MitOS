/*
	File: Source/MitOS/method/mtask.c
...
struct TASK *task_init(struct MEMMAN *memman)
{
	...
	timer_settime(tasktimer, task->priority);
...
void task_sleep(struct TASK *task)
{
	...
	if (task->flags == 2) {
		/* 处于活动状态 */
		now_task = task_getnow();
		task_remove(task);
		if (task == now_task) {
			/* 如果是让自己休眠，则需进行任务切换 */
			task_switchsub();
			now_task = task_getnow(); /* 在设定后获取当前任务的值 */
...
void task_switch(void)
{
	...
	timer_settime(tasktimer, new_task->priority);
