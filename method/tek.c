/*
	File: Source/MitOS/console/tek.c
...
static int tek_decmain5(int *work, UCHAR *src, int osiz, 
						UCHAR *q, int lc, int pb, int lp, int flags);
...
static int tek_rdget1(struct tek_STR_RNGDEC *rd, tek_TPRB *prob0, 
					  int n, int j, struct tek_STR_BITMODEL *bm)
{
	...
	do {
		...
		if (bm->lt > 0) {
			if (--bm->lt == 0) {
				if (tek_rdget1(rd, &rd->probs.fchglt, 
							   0x71, 0, &rd->bm[3]) == 0) {
err:
					longjmp(rd->errjmp, 1);
				}
				i = bm - rd->bm;
				if ((bm->s = tek_rdget1(rd, &rd->probs.fchgprm[i * 2 + bm->s], 
					0x71, 0, &rd->bm[1])) == 0) {
					i = tek_rdget1(rd, rd->probs.tbmt, 
								   0x74, 1, &rd->bm[2]) & 15;
					if (i == 15)
						goto err;
					tek_setbm5(bm, i, ((tek_rdget1(rd, rd->probs.tbmm, 0x74, 
							   1, &rd->bm[2]) - 1) & 15) + 1);
				}
				bm->lt = bm->lt0;
			}
...
static int tek_getlen5(struct tek_STR_RNGDEC *rd, int m, int s_pos, int stk)
{
	...
	if (tek_rdget1(rd, &rd->probs.lensel[m][0], 0x71, 0, rd->ptbm[3]) ^ stk)
		i = tek_rdget1(rd, rd->probs.pb[s_pos].lenlow[m], 
					   0x73, 1, rd->ptbm[4]) & 7;
	else if (tek_rdget1(rd, &rd->probs.lensel[m][1], 0x71, 0, rd->ptbm[3]) ^ stk)
		i = tek_rdget1(rd, rd->probs.pb[s_pos].lenmid[m], 
					   0x73, 1, rd->ptbm[5]);
	else {
		...
		if (i > 0) {
			if (i < 6 && stk == 0)
				i = tek_rdget1(rd, &rd->probs.lenext[(1 << i) - 2], 
							   i | 0x70, 1, rd->ptbm[7]) - 1;
			else
						...
						UCHAR *q, int lc, int pb, int lp, int flags)
{
	...
	int i, j, k, pmch, rep[4], s;
	int pos, m_pos = (1 << pb) - 1, m_lp = (1 << lp) - 1;
	...
	if ((tek_rdget1(rd, &prb->pb[0].st[0].mch, 
					0x71, 0, rd->ptbm[0]) ^ stk) == 0)
		goto err;
	*q++ = tek_rdget1(rd, prb->lit, lit0cntmsk, 1, &rd->bm[24]) & 0xff;
	pmch &= 0; s &= 0; pos = 1;
	while (pos < osiz) {
		s_pos = pos & m_pos;
		if (tek_rdget1(rd, &prb->pb[s_pos].st[s].mch, 
					   0x71, 0, rd->ptbm[s > 0]) ^ stk) {
			i = (q[-1] >> lcr | (pos & m_lp) << lc) << 8;
			s = state_table[s];
			if (pmch == 0)
				*q = tek_rdget1(rd, &prb->lit[i], lit0cntmsk, 1, &rd->bm[24]) 
					& 0xff;
			else {
				...
				pmch = q[rep[0]];
				do {
					j += j + tek_rdget1(rd, &lit1[(i + j) << 1 | pmch >> 7], 
										0x71, 0, rd->ptbm[2]);
					k--;
					if ((k & (lit0cntmsk >> 4)) == 0)
						bm++;
					if ((((pmch >> 7) ^ j) & 1) != 0 && k != 0) {
						j = tek_rdget1(rd, &prb->lit[i + j - 1], 
							k | (lit0cntmsk & 0x70), j, bm);
						break;
					}
					pmch <<= 1;
				} while (k);
			...
			if (tek_rdget1(rd, &prb->st[s].rep, 0x71, 0, rd->ptbm[13]) ^ stk) {
				...
				if (j >= 4)
					j = 3;
				rep[0] = j = tek_rdget1(rd, prb->pslot[j], 0x76, 1, 
										rd->ptbm[8 + (j == 3)]) & 0x3f;
				if (j >= 4) {
					k = (j >> 1) - 1;
					rep[0] = (2 | (j & 1)) << k;
					if (j < 14)
						rep[0] |= tek_revbit(tek_rdget1(rd, 
							&prb->spdis[j & 1][(1 << k) - 2], 
							k | 0x70, 1, rd->ptbm[10 + (k >= 4)]), k);
					else {
						if (stk == 0) {
							if (k -= 6)
								rep[0] |= tek_rdget0(rd, k, ~0) << 6;
							rep[0] |= tek_revbit(tek_rdget1(rd, 
								prb->algn, 0x76, 1, rd->ptbm[12]), 6);
						} else {
							rep[0] |= tek_rdget0(rd, k - 4, ~0) << 4;
							rep[0] |= tek_revbit(tek_rdget1(rd, 
								prb->algn, 0x74, 1, rd->ptbm[12]), 4);
						}
					}
				}
				rep[0] = ~rep[0];
			} else {
				if (tek_rdget1(rd, &prb->st[s].repg0, 0x71, 0, 
					rd->ptbm[13]) ^ stk) {
					i |= -1;
					if (tek_rdget1(rd, &prb->pb[s_pos].st[s].rep0l1, 
								   0x71, 0, rd->ptbm[13]) == 0) {
						s = s < 7 ? 9 : 11;
						goto skip;
					}
				} else {
					if (tek_rdget1(rd, &prb->st[s].repg1, 
								   0x71, 0, rd->ptbm[13]) ^ stk)
						i = rep[1];
					else {
						if (tek_rdget1(rd, &prb->st[s].repg2, 
									   0x71, 0, rd->ptbm[13]) ^ stk)
							i = rep[2];
						else {
							if (stk == 0) {
								if  (tek_rdget1(rd, &prb->repg3, 
												0x71, 0, &rd->bm[22]) == 0)
									goto err;
							}
							i = rep[3];
